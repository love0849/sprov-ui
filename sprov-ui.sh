...
install() {
    install_base
    bash <(curl -L -s https://github.com/love0849/sprov-ui/raw/last/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
uninstall() {
    ...
    echo -e "Github issues: ${green}https://github.com/love0849/sprov-ui/issues${plain}"
...
install_bbr() {
    bash <(curl -L -s https://github.com/love0849/blog/raw/last/bbr.sh)
    if [[ $? == 0 ]]; then
        echo ""
...
update_shell() {
    wget -O /usr/bin/sprov-ui -N --no-check-certificate https://github.com/love0849/sprov-ui/raw/last/sprov-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
